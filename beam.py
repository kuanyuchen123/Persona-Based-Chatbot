...
def beam_decode(decoder, encoder_hidden, src_pad_mask, device):
    beam_width = 15
    topk = 1
    decoded_batch = []
    ...
    for idx in range(1):
        ...
        while True:
            ...
            tgt_mask = decoder.get_tgt_mask(y_input.size(1)).to(device)
            tgt_pad_mask = decoder.create_pad_mask(y_input, params.PAD).to(device)
            ...
            _, decoder_output = decoder(encoder_hidden, y_input, tgt_mask, tgt_pad_mask, src_pad_mask)
